                        ...
                        <div class="search-form__wrapper">
                            <div class="input-group search-form__query">
                                <label>
                                    ...
                                    <input type="text" required name="search" id="search-name"
            ...
            <div class="spring">
                        ...
                        <div class="grid mb-30">
                            ...
                            <div class="grid__item grid__item--1/3 grid__item--middle hide--sm hide--md">
                                <p class="heading heading--5 heading--center heading--leading heading--trailing">
                                    Showing
                                    <strong>
                                        <?php echo $usecases_listing_display_count; ?> of <?php echo $usecases_listing_total_count; ?>
                                    </strong>
                                    Use Cases
                                </p>
                            </div>
                ...
                <table class="table table--alt table--list hide--md hide--sm">
                    ...
                    <tbody class="table__body" id="product-listing-table">
                      <?php foreach ($usecases_listing as $keys => $usecases_listing_node) { ?>
                        <tr class="table__tr">
                            <td class="table__td">
                                <a href="/<?php echo  drupal_get_path_alias('node/'.$usecases_listing_node->nid); ?>" class=""><?php echo  $usecases_listing_node->node_title; ?></a>
                            </td>
                            <td class="table__td">
                                <?php 
                                    $api_croosrespondence = $api_north = $api_north_seemore = array();
                                    if(!empty($usecases_listing_node->_field_data['nid']['entity']->field_apis_reference)){ 
                                        $k=1;$i=1; 
                                        foreach ($usecases_listing_node->_field_data['nid']['entity']->field_apis_reference['und'] as $key_apis => $value_apis) {
                                            $link_node = node_load($value_apis['target_id']);
                                            if ($link_node->field_api_type['und'][0]['value'] == 'south'){
                                               $api_croosrespondence_mobile[$usecases_listing_node->nid][$link_node->nid] = $link_node->title;  
                                               if($i < 3 ) $api_croosrespondence['normal'][$link_node->nid] = $link_node->title;
                                               else $api_croosrespondence['seemore'][$link_node->nid] = $link_node->title;
                                               $k--;
                                            }else{
                                               $api_north_mobile[$usecases_listing_node->nid][$link_node->nid] = $link_node->title; 
                            ...
                            <td class="table__td">
                                <?php 
                                    ...
                                    if(!empty($usecases_listing_node->_field_data['nid']['entity']->field_microservices_reference)){ 
                                        $k=1;
                                        foreach ($usecases_listing_node->_field_data['nid']['entity']->field_microservices_reference['und'] as $key_apis => $value_apis) {
                                            $link_node = node_load($value_apis['target_id']);
                                            $microservices_node_mobile[$usecases_listing_node->nid][$link_node->nid] = $link_node->title;
                                            if($k < 3 ) $api_north[$link_node->nid] = $link_node->title;
                                            else $api_north_seemore[$link_node->nid] = $link_node->title;
                                            $k++;
                                        }
                            ...
                            <td class="table__td">
                                <?php 
                                    $api_north = $api_north_seemore = array();
                                    if(!empty($usecases_listing_node->_field_data['nid']['entity']->field_channel_reference)){ 
                                        $k=1;
                                        foreach ($usecases_listing_node->_field_data['nid']['entity']->field_channel_reference['und'] as $key_apis => $value_apis) {
                                            $link_node = node_load($value_apis['target_id']);
                                            $channel_node_mobile[$usecases_listing_node->nid][$link_node->nid] = $link_node->title;
                                            if($k < 3 ) $api_north[$link_node->nid] = $link_node->title;
                                            else $api_north_seemore[$link_node->nid] = $link_node->title;
                                            $k++;
                                        }
                            ...
                            <td class="table__td">
                                <?php echo count($usecases_listing_node->field_field_markets);?>
                            </td>
                            <td class="table__td">
                                    <ul>
                                    <?php if(!empty($api_croosrespondence['normal'])) { foreach ($api_croosrespondence['normal'] as $key => $value) {?>
                                        <li>
                                            <a href="/<?php echo drupal_get_path_alias('node/'.$key); ?>"><?php echo $value; ?></a>
                                        </li>
                                    <?php } } ?>
                                    <div data-js="_accordion" data-classes="active=show-more__heading--active"
                                        class=" js-accordion show-more">
                                        <div class="js-accordion-content show-more__content show-more__content--collapse ">
                                            <?php if(!empty($api_croosrespondence['seemore'])) { foreach ($api_croosrespondence['seemore'] as $key => $value) {?>
                                                <li>
                                                    <a href="/<?php echo drupal_get_path_alias('node/'.$key); ?>"><?php echo $value; ?></a>
                                                </li>
                                            <?php }} ?>
                                        </div>
                                        <?php if( !empty($api_croosrespondence['seemore']) ) {?>
                                        <button class="js-accordion-heading show-more__heading">
                                            <span class="show-more__heading-text">
                                                see
                                                <span class="show-more__heading-more">
                ...
                <div class="js-accordion accordion accordion--spaced hide--lg align--left" data-js="_accordion">
                        ...
                        <!-- ACCORDION ITEM 1 | HEADING -->
                      <?php foreach ($usecases_listing as $usecases_listing_m) { ?>

                        <div class="js-accordion-item accordion__item">
                            <h3 class="js-accordion-heading accordion__heading">
                                <span class="chevron">
                                    <span class="chevron__text">
                                        <?php echo  $usecases_listing_m->node_title; ?>
                                    </span>
                                    <span class="chevron__container accordion__chevron">
                            ...
                            <div class="js-accordion-content accordion__content accordion__content--wide accordion__content--collapse ">
                                <!-- SUBGROUP 1 -->
                                <?php if(!empty($microservices_node_mobile[$usecases_listing_m->nid])){ ?>
                                <div class="accordion__content--subgroup">
                                    <!-- SUBGROUP TITLE -->
                                    <p class="accordion__content--header">
                                        Microservice
                                    </p>
                                    <?php foreach ($microservices_node_mobile[$usecases_listing_m->nid] as $key_mobile => $value_mobile) {?>                                    
                                ...
                                <?php } ?>
                                <!-- SUBGROUP 2 -->
                                <?php if(!empty($api_north_mobile[$usecases_listing_m->nid])){ ?>
                                <div class="accordion__content--subgroup">
                                    <!-- SUBGROUP TITLE -->
                                    <p class="accordion__content--header">
                                        API
                                    ...
                                    <!-- LINK TILE -->
                                    <?php foreach ($api_north_mobile[$usecases_listing_m->nid] as $key_mobile => $value_mobile) {?>
                                    <a href="/<?php echo drupal_get_path_alias('node/'.$key_mobile); ?>" class="link link--tile link--tile--accordion">
                                        <span class="chevron">
                                            <span class="chevron__text">
                                                <span class="media media--middle">
                                ...
                                <?php } ?>
                                <!-- SUBGROUP 3 -->
                                <?php if(!empty($channel_node_mobile[$usecases_listing_m->nid])){ ?>
                                <div class="accordion__content--subgroup">
                                    <!-- SUBGROUP TITLE -->
                                    <p class="accordion__content--header">
                                        Channels
                                    ...
                                    <!-- LINK TILE -->
                                    <?php foreach ($channel_node_mobile[$usecases_listing_m->nid] as $key_mobile => $value_mobile) {?>                                    
                                    <a href="/<?php echo drupal_get_path_alias('node/'.$key_mobile); ?>" class="link link--tile link--tile--accordion">
                                        <span class="chevron">
                                            <span class="chevron__text">
                                                <span class="media media--middle">
                                ...
                                <!-- SUBGROUP 5 -->
                                <?php if(!empty($api_croosrespondence_mobile[$usecases_listing_m->nid])){ ?>
                                <div class="accordion__content--subgroup">
                                    <!-- SUBGROUP TITLE -->
                                    <p class="accordion__content--header">
                                        API Correspondence
                                    ...
                                    <?php foreach ($api_croosrespondence_mobile[$usecases_listing_m->nid] as $key_mobile => $value_mobile) {?>
                                    <a href="/<?php echo drupal_get_path_alias('node/'.$key_mobile); ?>" class="link link--tile link--tile--accordion ">
                                        <span class="chevron">
                                            <span class="chevron__text">
                                                <span class="media media--middle">
                ...
                <?php if($usecases_listing_display_count >= 10 ) { ?>
                <div class="align--center">
                    <a href="/apis?page=<?php echo $limit; ?>" class="button button--primary mt-40">
                        Load more
                    </a>
                </div>
                <?php } elseif($usecases_listing_display_count == 0) { ?>
                <div class="align--center">
